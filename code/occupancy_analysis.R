...
spp_colext <- spp_occ_deltas %>%
  ...
  mutate(rte_col = map_dbl(data, ~{
    df <- .
    n_distinct(df$stateroute[df$d1 == 0 & df$d2 > 0])/n_distinct(df$stateroute)
  }),
  ...
  rte_dTrans = map_dbl(data, ~{
    df <- .
    trans_d1 <- n_distinct(df$stateroute[df$d1 < 0.3 & df$d1 > 0])/n_distinct(df$stateroute)
    trans_d2 <- n_distinct(df$stateroute[df$d2 < 0.3 & df$d2 > 0])/n_distinct(df$stateroute)
    trans_d2-trans_d1
  }),
